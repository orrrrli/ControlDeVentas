...
<template>
    <v-layout align-start>
        <v-flex>
            <h3 class="text-center mb-5 primary--text">Consultar Ventas</h3>
            ...
            <v-container v-if="verFormularioNuevoVenta == 1" grid-list-sm class="pa-4 white">
                <v-layout row wrap>
                    ...
                    <v-flex xs12 sm6 md6 lg3 x13>
                        <v-select v-model="IdPersona" :items="ListaDeCliente" label="Seleccione un Cliente" disabled></v-select>
                    </v-flex>
...
export default {
  ...
  methods: {
    ...
    ListadoVentas () {
      const me = this
      if (this.fechaFinal === '') {
        axios.get('api/Ventas/ListarVentas/').then(function (response) {
        ...
        axios.get('api/Ventas/ListadoRangoFechas/' + this.fechaInicial + '/' + this.fechaFinal).then(function (response) {
          me.ListaDeVentas = response.data
          console.log(me.ListaDeVentas)
        }).catch(function (error) {
          console.log(error)
